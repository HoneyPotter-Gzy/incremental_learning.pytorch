...
@Time: 2022/6/6 20:46  
@Author: Zheyuan Gu
@File: incremental_finetune_train.py
@Description: None
'''
...
class IncrementalFinetuneMethod():
    '''
    增量学习中的微调方法，提供旧类别的训练和新类别在旧类别上的微调。
    load_dataset(): 加载data_loader
    train(): 训练
    incrementa_train()：增量训练
    test(): 仅测试
    '''
    ...
    def __init__ (self, class_num, epoch, batch_size, learning_rate,
        ...
        self.model = get_MyIncrementalResnet(class_num)
        self.epoch = epoch
    ...
    def train (self):
        ...
        save_plots(train_acc_list, test_acc_list, train_loss_list,
                   test_loss_list, title = self.model_name)
    ...
    def increment_class(self, n):
        self.model.module.increment_classes(n)
        self.model.module.to(device = self.device)
        print("model.fc modified successfully")
