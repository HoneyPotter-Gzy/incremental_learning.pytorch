...
tsne_2D = manifold.TSNE(n_components=2, init='pca', random_state=rs, learning_rate = lr, perplexity= pp) #调用TSNE
result_2D = tsne_2D.fit_transform(train_img_path)

x=np.savetxt('origin_lr{}_pp{}_rs{}.txt'.format(lr, pp, rs), result_2D)

print('Finished......')

#调用上面的两个函数进行可视化
fig1 = plot_embedding_2D(result_2D, label,'t-SNE')
plt.savefig(r'origin_lr{}_pp{}_rs{}.png'.format(lr, pp, rs))
plt.show()
